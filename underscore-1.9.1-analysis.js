...
(function () {
  ...
  /**
   * 建立根对象，其实就是缓存不同环境里的全局对象，在浏览器环境下，为了兼容web worker,用self缓存。
   * 因为在webworker创建的子线程中无法访问window全局对象，self在webworker是子线程的全局变量，在主线程中self是全局对象window的一个属性，是对window自身的一个引用
   * 关于webworker 可以看看阮一峰的博客 http://www.ruanyifeng.com/blog/2018/07/web-worker.html
   * 
   * 以下赋值逻辑是 判断self是否为对象，再判断self的self属性是否为自身的引用，如果前两者条件成立且self存在，赋值为self
   * 同理于其他环境的全局变量缓存
   * 这里不太理解为什么要去判断self自身的self属性为自身的引用 也不能理解为什么在this之后还要做一个空对象的判断处理
   */
  ...
  /**
   * 原生的isNaN对于undefined的判定isNaN(undefined) === true
   * 因此为了处理这种情况 判断一个值是否为NaN 需要满足这个是数值 
   * NaN实际上是数值类型 Object.prototype.toString.call(NaN) === '[object Number]'
   * underscore中，将Object.prototype.toString方法用toString缓存了
   */ 
